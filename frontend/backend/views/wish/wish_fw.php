...
<input type="hidden" class="search_params" name="type" value="<?=html::encode($_GET['type'])?>">
<div class="Contant Mwidth ly_main">
            ...
            <div class="secmain-select">
                <div class="secmain-select-type">
                    ...
                    <ul class="secmain-type">
                        <li  class=""><a data-type="0" href="/wish/wish_fw?type=0">在职党员报到登记</a></li>
                        <li  class=""><a data-type="3" href="/wish/wish_fw?type=3">我需要</a></li>
                        <li class=""><a data-type="1" href="/wish/wish_fw?type=1">我提供</a></li>
                        <li class=""><a data-type="2" href="/wish/wish_fw?type=2">开展社区服务活动</a></li>
                    </ul>
                    <a href="/wish/wish_add?type=<?=$_GET['type'] ?>" target="_blank" class="btn btn-info" style=" float: right;margin-right: 4%!important; margin: 10px auto;"><?=(($_GET['type']==0||$_GET['type']==3)?'许愿':'添加')?></a>
                </div>
                <?php
                if($_GET['type']==0){
                    echo '
            ...
            <ul class="secmain-list wish">
                ...
                if(!empty($data['result'])){
                    foreach ($data['result'] as $num =>$val){
                        echo'
                           <li>
                            <a target="_blank"  href="/wish/wish_detail?idStr='.html::encode($val['idStr']).'&type='.html::encode($_GET['type']).'">
                                <div class="txt">'.html::encode($val['title']).'</div>
                                <div class="adds">'.html::encode($val['content']).'</div>
                                <div class="num"><span class="rnum ';if(isset($val['wishState'])){echo($val['wishState']==1?'bg-wish-state-red':'bg-wish-state-yellow') ;} echo '">'; if(isset($val['wishState'])){echo common\models\commonEnum::getWishState($val['wishState']); }; echo '</span><span class="pnum">'.html::encode($val['contact']).'</span></div>
                                <i></i>
                            </a>';
                        if($_GET['type']==2){
                            echo '<button class="btn btn-danger btn-pad" data-id="'.html::encode($val['idStr']).'" onclick="removeWishiper(this)">删除</button>';
                        }
                        if($_GET['type']==3){
