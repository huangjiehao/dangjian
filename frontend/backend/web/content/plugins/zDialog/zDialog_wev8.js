...
var Dialog = function (currentwin) {
    ...
    this.okButton = null;
    this.e8SepLine = null;
    this.okLabel = "确定";//SystemEnv.getHtmlNoteName(3451)
    this.cancelLabel = "取消";//SystemEnv.getHtmlNoteName(3516)
  ...
  <table id="_DialogTable_' + this.ID + '"  width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0"  style=" font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "")+(this.hideDraghandle ? "display:none;" : "") + '" class="' + this.dialogtitleclass + '">\
      <td height="30px;" style="background-color:#4f81bd;"><div style="float: left;color:#fff;"><span style="width:10px;display:inline-block;"></span><span id="_Title_' + this.ID + '" style="font-size:13px;height:30px;line-height:30px;"> ' +  this.Title  + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.background=\'url(\/content/images/16_close_wev8.png\)\'" onmouseover="this.style.background=\'url(\/content/images/16_close_wev8.png\)\'" style="font-family:Verdana!important;\
        				margin: 10px 8px 0; font-size:8px!important;position: relative; cursor: pointer; float: right; height: 16px; width: 16px;background: url(\/content/images/16_close_wev8.png\);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div>';
        if(this.maxiumnable){				
			html += '\
		...
		margin: 10px 8px 0; font-size:8px!important;position: relative; cursor: pointer; float: right; height: 16px; width: 16px;background: url(\/content/images/max2_wev8.png\);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div>';
		}
		html += '</td>\
    </tr>\
    <tr valign="top">\
...
Dialog.prototype.mouseOut = function(obj){
	if(this.maxState){//最大化状态
		jQuery(obj).css("background","url(\/content/images/recovery_wev8.png\)");
	}else{
		jQuery(obj).css("background","url(\/content/images/max2_wev8.png\)");
	}
}
...
Dialog.prototype.maxOrRecoveryWindow = function(obj){
	...
	if(!this.maxState){
		...
		this.maxState = true;
		jQuery(obj).css("background-image","url(/content/images/recovery_wev8.png)");
	}else{
