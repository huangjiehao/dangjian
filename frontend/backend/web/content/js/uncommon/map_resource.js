...
function initMarker() {//mapgis,myIcon
    ...
    for (var k = 0; k < mygis.length; k++) {
        var lat = mygis[k].latitude;
        var lon = mygis[k].longitude;
        ...
        var myLabel = new BMap.Label(mygis[k]['title'], {offset: new BMap.Size(6, -15), position: point});
...
function openOrgInfoWindow(marker) {
    ...
    var mapValue = {};
    mapValue.title = marker.gisInfo.title;
    mapValue.address = marker.gisInfo.address;
    mapValue.id = marker.gisInfo.id;
...
$(document).on("click", ".node_name", function () {
    ...
    myMarker.gisInfo = {address: address, name: name, idStr: idStr};
